...
from porespy.tools import _insert_ellipse_at_points
...
__all__ = [
    ...
    "pseudo_gravity_packing_ellipsoids"
]
...
def pseudo_gravity_packing_ellipsoids(
    im,
    rA: int,
    rB: int,
    rC: int,
    ...
    """
    logger.debug(f"rsa: Adding ellipsoids of size {rA}, {rB}, {rC}")

    if seed is not None:  # Initialize rng so numba sees it
        _set_seed(seed)
        np.random.seed(seed)

    # Swap axis and radii if necessary
    im = np.swapaxes(im, 0, axis)
    im_temp = np.zeros_like(im, dtype=bool)
    if axis == 2:
        rA, rB, rC = rC, rB, rA  # Swap dimensions
        theta_x, theta_y, theta_z = theta_z, theta_y, theta_x
    elif axis == 1:
        rA, rB, rC = rA, rC, rB  # Adjust for y-priority
        theta_x, theta_y, theta_z = theta_x, theta_z, theta_y

    # Adjust radii for x, y, and z
    rx, ry, rz = rA - 1, rB - 1, rC - 1  
    
    # Define anisotropic structuring element
    if im.ndim == 2:
        strel = lambda rx, ry: np.ones((2 * rx + 1, 2 * ry + 1), dtype=bool)
    else:
        strel = lambda rx, ry, rz: np.ones((2 * rx + 1, 2 * ry + 1, 2 * rz + 1), dtype=bool)
    
    # Perform anisotropic erosion
    sites = fftmorphology(im >= 1, strel=strel(rx, ry, rz), mode='erosion')
    
    # Define inlets (affecting the negative boundary in the x-axis)
    inlets = np.zeros_like(im)
    inlets[-(rx + 1), :, :] = True  # Now inlets only apply along the x-boundary
    
    # Remove disconnected regions
    sites = trim_disconnected_blobs(im=sites, inlets=inlets)
    
    # Get the minimum x-boundary where erosion is still active
    x_min = np.where(sites)[0].min()
    n = None
    angles = None  # Initialize angles before loop

    for n in tqdm(range(maxiter), **settings.tqdm):
        # Get valid sites based on nonzero values within a 3D region
        if im.ndim == 2:
            x, y = np.where(sites[x_min:x_min + 2 * rx, :])  # Uses rx, ry for 2D
        else:
            x, y, z = np.where(sites[x_min:x_min + 2 * rx, :, :])  # Uses rx, ry, rz for 3D
    
        # Stop if no more active sites remain
        if len(x) == 0:
            break
    
        # Select from the lowest x-values
        options = np.where(x == x.min())[0]
        choice = np.random.randint(len(options))
    
        # Store the selected center, considering rx, ry, and rz
        if im.ndim == 2:
            cen = np.vstack([x[options[choice]] + x_min,
                              y[options[choice]]])
        else:
            cen = np.vstack([x[options[choice]] + x_min,
                              ...
                              z[options[choice]]])
        sites[
            max(0, cen[:, 0].astype(int)[0] - rx): min(im.shape[0], cen[:, 0].astype(int)[0] + rx + 1),
            max(0, cen[:, 0].astype(int)[1] - ry): min(im.shape[1], cen[:, 0].astype(int)[1] + ry + 1),
            max(0, cen[:, 0].astype(int)[2] - rz): min(im.shape[2], cen[:, 0].astype(int)[2] + rz + 1)
        ] = False  # Remove selected voxel and surrounding region
        ...
        im_temp, angles = _insert_ellipse_at_points(im=im_temp,
                                                    coords=cen,
                                                    # coords=cen_adjusted,
                                                    rA = rA-clearanceA,
                                                    rB = rB-clearanceB,
                                                    ...
                                                    theta_y=theta_y,
                                                    theta_z=theta_z)
        sites, option_angles = _insert_ellipse_at_points(im=sites,
                                                        coords=cen,
                                                        # coords=cen_adjusted,
                                                        rA=2*rA,
                                                        rB=2*rB,
...
#                                                     coords=cen,
#                                                     rA = r - clearance,
#                                                     rB = int(0.8*(r - clearance)),
#                                                     rC = int(0.2*(r - clearance)),
#                                                     v=True,
#                                                     overwrite=True,
#                                                     smooth=True,
#                                                     rotation_angles=True,
...
#                                                         coords=cen,
#                                                         rA=2*r,
#                                                         rB=2*int(0.8*(r - clearance)),
#                                                         rC=2*int(0.2*(r - clearance)),
#                                                         v=0,
#                                                         smooth=True,
#                                                         overwrite=True,
#                                                         rotation_angles=True,
...
if __name__ == "__main__":
    ...
    # im = ps.generators.pseudo_gravity_packing_ellipsoids(im=np.zeros(shape, dtype=bool),
    #                                                      rA=7, rB=5, rC=3, clearanceA=3,
    #                                                      clearanceB=3, clearanceC=3,
    #                                                      edges='contained', seed=0)
    ...
    im = ps.generators.pseudo_electrostatic_packing(im=np.ones(shape,
                                                    ...
                                                    clearance=4,
                                                    maxiter=50, seed=0)
    ax[1].imshow(im[:,:,20], origin='lower')
