...
from porespy.tools import _insert_ellipse_at_points_periodic, _insert_ellipse_at_points
...
def rsa_ellipsoids(
    ...
    r"""
    Generates a ellipsoid or ellipse packing using Random Sequential Addition
    ...
    im_or_shape : ndarray or list
        ...
        empty image.  In both cases the spheres are added as ``True`` values
        to the background.  Since ``True`` is considered the pore space, then
        the added ellipsoids represent holes.
    rA, rB, rC : int
        The radius of the ellipse or ellipsoid to insert.
    volume_fraction : scalar (default is 1.0)
        The fraction of the image that should be filled with spheres.  The
        spheres are added as ``True``'s, so each sphere addition increases the
        ``volume_fraction`` until the specified limit is reached.  Note that if
        ...
        ``mode`` is ``'extended'``.
    clearance : int (optional, default = 0)
        The amount of space to put between each ellipsoid. Negative values are
        acceptable to create overlaps, so long as ``abs(clearance) < r``.
    protrusion : int (optional, default = 0)
        The amount by which inserted spheres are allowed to protrude outside of
        the given background.  If set to 0 (the default) then all ellipsoids will
        be fully inside the region marked ``False`` in the input image. If > 0, then
        ellipsoids will extend into the region marked ``True`` in the input image.
    n_max : int (default is 100,000)
        The maximum number of ellipsoids to add.  Using a low value may halt
        the addition process prior to reaching the specified
        ``volume_fraction``.  If ``None`` is given, then no limit is used.
    mode : string (default is 'contained')
        Controls how the edges of the image are handled.  Options are:
        ...
        'contained'
            ellipsoids are all completely within the image
        'extended'
            ellipsoids are allowed to extend beyond the edge of the
            image.  In this mode the volume fraction will be less than
            requested since some spheres extend beyond the image, but their
            entire volume is counted as added for computational efficiency.
        'periodic'
            the part of the ellipsoids that extend beyond the edge of the image
            are inserted in the other side of the image.
    ...
    """
    logger.debug(f"rsa: Adding spheres of size {rA}, {rB}, {rC}")
    if np.array(im_or_shape).ndim < 2:
        im = np.zeros(shape=im_or_shape, dtype=bool)
        input_im = np.copy(im)
    else:
    ...
    vf_final = volume_fraction
    vf_start = im.sum(dtype=np.int64) / im.size
    vf_template = ps_round(max([rA, rB, rC]), ndim=im.ndim, smooth=smooth).sum(dtype=np.int64) / im.size
    logger.debug(f"Initial volume fraction: {vf_start}")
    # Dilate existing objects by strel to remove pixels near them
    # from consideration for sphere placement
    logger.info("Dilating foreground features by sphere radius")
    if mode=="periodic":
        im_periodic=np.pad(im, pad_width=max([rA, rB, rC]), mode="wrap")
        dt_periodic = edt(im_periodic == 0)
        dt = dt_periodic[ int(max([rA, rB, rC])):len(dt_periodic[1, :])-int(max([rA, rB, rC])), int(max([rA, rB, rC])):len(dt_periodic[:, 1])-int(max([rA, rB, rC]))]
    else:
        dt = edt(input_im == 0)
    # if im.ndim == 3:
    #     if rotation_angles:
    ...
    #         len_x = max(rA, rB * cos_x - rC * sin_x, rA * cos_y + rC * sin_y)
    #         len_y = max(rB * cos_x - rC * sin_x, rB, rA * cos_z + rB * sin_z)
    #         len_z = max(rB * sin_x + rC * cos_x, rA * cos_y + rC * sin_y, rC)

    #         # options_im[:,:,:] = dt >= (rA_y - protusion)
    #         options_im_x = dt >= (len_x - protrusion_x)
    #     else:
    #         options_im = dt >= (int(max([rA, rB, rC])) - protrusion)
    # options_im = dt >= (int(min([rA, rB, rC])) - protrusion)
    options_im = dt >= (int(max([rA, rB, rC])) - protrusion)
    # Depending on mode, adjust options_im to remove options around edge
    if mode == "contained":
        border = get_border(im.shape, thickness=max([rA, rB, rC]), mode="faces")
        options_im[border] = False
    elif mode == "extended":
        im = np.pad(im, pad_width=min([rA, rB, rC]), mode="edge")
        options_im = np.pad(options_im, min([rA, rB, rC]), mode='symmetric')
    elif mode == "periodic":
        options_im=options_im
    else:
        raise Exception("Unrecognized mode: ", mode)
    # Begin inserting the spheres
    vf = vf_start
    ...
    while (vf <= vf_final) and (i < n_max) and (len(free_sites) > 0):
        ...
        if all(np.array(c) == -1):
            break
        if mode == "periodic":
            im, angles = _insert_ellipse_at_points_periodic(im=im,
                                                            coords=np.vstack(c),
                                                            rA = rA,
                                                            rB = rB,
                                                            rC = rC,
            ...
            options_im, option_angles = _insert_ellipse_at_points_periodic(im=options_im,
                                                                            coords=np.vstack(c),
                                                                            rA=1,
                                                                            rB=1,
                                                                            rC=1,
                                                                            ...
                                                                            smooth=smooth,
                                                                            overwrite=True,
                                                                            rotation_angles=True,
                                                                            theta_x=angles[0],
                                                                            theta_y=angles[1],
                                                                            theta_z=angles[2])
            
        else:
            im, angles = _insert_ellipse_at_points(im=im,
                                                    coords=np.vstack(c),
                                                    rA = rA,
                                                    rB = rB,
                                                    rC = rC,
            ...
            # options_im = _insert_disk_at_points(im=options_im,
            #                                               coords=np.vstack(c),
            #                                               r=2*max(rA,rB,rC) + int(np.round(clearance / 2)),
            #                                               v=False,
            #                                               smooth=smooth,
            #                                               overwrite=True)
            # options_im, option_angles = _insert_ellipse_at_points(im=options_im,
            #                                                     coords=np.vstack(c),
            #                                                     rA=2*rA + int(np.round(clearance / 2)),
            #                                                     rB=2*rB + int(np.round(clearance / 2)),
            #                                                     rC=2*rC + int(np.round(clearance / 2)),
            #                                                     v=True,
            #                                                     smooth=smooth,
            #                                                     overwrite=True,
            #                                                     rotation_angles=True,
        ...
        vf += vf_template
        i += 1
        # print(f"Radio: {rA}, clearance: {clearance}")
    logger.info(f"Number of spheres inserted: {i}")
    im = extract_subsection(im, shape_orig)
    logger.debug("Final volume fraction:", vf)
    if not return_spheres:
